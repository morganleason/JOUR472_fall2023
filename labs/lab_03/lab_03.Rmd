...
md_payments <- read_csv("data/State_of_Maryland_Payments_Data__FY2008_to_FY2024.csv", guess_max=10) |> clean_names()
...
A1: I found 277 records of rows where the vendor name contains the county's school system.

```{r}
pg_schools <- md_payments |>
  filter(str_detect(vendor_name, 'Prince Georges County School') | str_detect(vendor_name, 'PG County Public') | str_detect(vendor_name, 'Prince Georges Public') | str_detect(vendor_name, 'PG'))
```
...
A2: 2014 had the largest total amount. I am wondering why 2024 has significantly less spending than 2014.
...
A3. The Maryland Department of Health, State Department of Education, and the Department of Human Services all have multiple payments under the same name and should be combined into one. One way to clean the data is to use the "get_dupes" function to find duplicates of the same vendors.

```{r}
md_payments |>
  get_dupes(vendor_name) |>
...
A4: Stem cell research funding has mostly increased gradually throughout the years, but peaked in 2020.

```{r}
md_grants_loans |>
  filter(str_detect(description, 'Stem Cell Research') | fiscal_year) |>
  group_by(fiscal_year) |>
