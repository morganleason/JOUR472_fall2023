...
**Q1.** Maryland voters made history in 2022 by electing the state's first two African-American statewide office holders: **Wes Moor**e as governor and **Anthony Brown** as attorney general. You want to compare the two's electoral performances. Using the `md_statewide_county.csv` file in the data folder, calculate the **total number of votes** for both the governor's race and the attorney general's race, then percentages for both Moore and Brown *based on the total of their respective contests*. **Then calculate the difference between their percentages.**

Get the geometry for Maryland's counties using tigris and, joining that dataframe to your results dataframe, make a county-level map of the differences between Moore and Brown's percentages. What is the story here, and what is the county with the most interesting results?

**A1. The story here is that Moore had more votes than Brown. The county with the most votes was Montgomery county.**
...
md_statewide <- read_csv("data/md_statewide_county.csv") |> clean_names() |>
  ...
  mutate(total_votes = cox + moore + lashar + wallace + harding + sefcik + gov_writein + peroutka + brown + ag_writein, pct_moore = (moore/total_votes)*100) |>
  mutate(total_votes = cox + moore + lashar + wallace + harding + sefcik + gov_writein + peroutka + brown + ag_writein, pct_brown = (brown/total_votes)*100) |>
  mutate(total_governor = cox + moore + lashar + wallace + harding + sefcik + gov_writein) |>
...
#Create the map
ggplot() +
  geom_sf(data=md_statewide_geometry,aes(fill=difference)) + 
  theme_minimal() + 
...
**A2. The potential story here is that**
...
county_outages_geometry <- md_counties |> 
  inner_join(county_outages_march2023, by=c('NAME'='county'))
...
ggplot() +
  geom_sf(data=county_outages_geometry,aes(fill=per_capita)) + 
  theme_minimal() + 
  scale_fill_gradient2()
...
ev_data_geometry <- md_counties |>
  inner_join(ev_2020_2023, by=c('NAME'='county'))

```

```{r}
#Map out the change
county_centroids <- st_centroid
...
ggplot() +
  geom_sf(data=county_ev_geometry,aes(fill=pct_change)) + 
...
ggplot() + 
  geom_sf(data = ev_population_data, aes(fill = per_capita)) +
  theme_minimal() +
  scale_fill_gradient2()
```
