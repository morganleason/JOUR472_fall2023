...
tabula_Baltimore_County_Carey_Samantha_log_OD \<- read_csv("Documents/J472/JOUR472_fall2023/labs/lab_05/data/tabula-Baltimore County; Carey, Samantha log OD.csv")
...
```{r}
Baltimore_County_log <- read_csv("data/tabula-Baltimore County; Carey, Samantha log OD.csv", col_names = FALSE) |> clean_names() |>
...
A1. July 14th has the most of these calls. Not every day is recorded, which leads me to believe there could be days when there were no overdose calls.
...
A2. The weekends have higher rates of calls.

```{r}
Baltimore_County_log |>  mutate(day_of_week = wday(date)) |>
  group_by(day_of_week) |>
  summarise(count = n()) |>
  mutate(pct_total = count/4112) |>
...
A3. Silver Spring Road, Windsor Mill road, and Susquehanna avenue had the most calls. I would describe these roads as being located in a demographically-diverse area with lower incomes. The structure of the original data in terms of there being mutliple entries for the same day on different streets, makes me less confident in the counts by location and date because we are not analyzing all neighborhoods and towns. Certain neighborhoods carry specific income levels and other traits that could potentially raise the counts in that area.
...
A4. I would write a investigative story into whether there is a correlation between weekends and higher rates of overdoses.
