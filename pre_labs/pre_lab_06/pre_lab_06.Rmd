...
**Task** Load the four CSV population estimate files. Do they have the same column structure? **Answer: yes a lot of the data has similar column structure, therefore we should combine them. However, I noticed that there are two total columns, one is capitalized and the other is non-capitalized and carries a list of N/A data.**
...
**Task** Run the following code to filter on Frederick County's registration data and look at the results. What stands out to you about the changes in party registration over time? **Answer: What stands out is that the Democratic registration jumps up much faster than the Republican registration.**
...
**Task** Run the following code to join Maryland population data and the 2022 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **Answer: There are 24 rows in the data. There should be 24.**
...
**Task** Load the Maryland demographic data and examine it. What kind of information does it contain? **Answer: The data incldues the population by age, race and ethnicity. It says the county names are "name" instead of county. The data will combine but it will create a new column that will not align with all of the data, making it difficult to analyze.**
...
**Answer: The county with the highest percentage of an 18+ black population and the highest percentage of a democratic population is Prince Georges county. The 18+ black population percentage is 60.90% and the democratic population is 77.09%.**
...
**Task** Now let's find the county with the highest 18+ Asian population as a percentage of all population 18 and over and also the percentage of Democrats in that county. How do these results compare to the previous task? Put your answers below. **Answer: The highest 18+ Asian population is in Howard county and carries a percentage of 19%. I noticed the Asian population is significantly smaller in all other counties. The highest democrat percentage is still in Prince George's county. Still, a decent percentage is located in Howard and carries a percentage of 52.57%.**
...
**Task** Run the following code to load the Zip Code Tabulation Area data for Maryland. What's the datatype of the ZCTA5N column? **Answer: The data type is a character.**
...
**Task** Run the following code to change the datatype of ZCTA5N from numeric to character. What's the datatype of ZCTA5N? **Answer: The datatype is a qualitative value representing a zip code.**
