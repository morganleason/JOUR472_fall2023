...
**Task** Run the following code to load data. Describe what you see in the answer space below. What columns exist in this data? **Answer: We have the spatial layout in the columns shape length and shape area. The multipolygon represents the geography of that zip code.**
...
**Task** Run the following code. Describe the output in the space below: what kind of information does it contain? **Answer: The information has to do with the exact locations of zipcodes, by their coordinates, in Maryland.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: The numbers not only describe the shape of the zip file, they describe the exact location they should be in the context of the world. Specific real-world coordinates create the puzzle of Maryland below.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: This time, we are looking at counties.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: Rather than having a few zip codes, we now have 3234 counties in the United States being displayed in the data. We have There are many more columns in this data as well (Counties can also mean cities, burrows, etc.).**
...
**Task** Run the following code. Describe the output in the space below. **Answer: This code filters the data into only Maryland counties.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: This code produces a map of just the Maryland territories by county and other municipalities.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: This code layers two maps together. The two geom_sf's call specific data and the counties in md_zips.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: Shows the number of foreclosures per zip code, specifically in March of 2020.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: This is the combined data of the zip codes with foreclosures and the data of the foreclosures specific to march of 2020.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: This ggplot created a map based on a shape file with the zip codes with foreclosures data, after the merge above. The map is filled with a certain color, using the march 2023 data that includes how to fill it. The scale is created by going into the dataframes and uses that data. There appears to be some clusters in the data by color.**
...
**Task** Run the following code. Describe the output in the space below. Be sure to input your census api key. **Answer: This is where we are adding things to the map to find patterns.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: We read the data and reshaped it so that it is not a bunch of columns for each month but condenses it into less columns. Then, we load the maryland population per county data, and when we use the get_acs function, we see the observational data from the Census and the Maryland community data.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: Now we can see both data sets that have the notices and the geometry.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: This just shows the two data sets.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: Now we are cleaning up the names in the data set to make it more concise.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: The two data sets are joined together.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: This one has added rate as one of the columns and arranged it by descending rate.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: Now we have a map of the rates per county.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: The last code included the names of the counties and had a slightly changing color range. This data uses the color range indicated by the data and keeps the data minimalistic-looking by removing the names of Maryland counties.**
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **Answer: This code changes the scale to make the color differences more measurable and clear to distinguish between.**
