...
**Answer: Making a new variable reorganizes the data so we can specifically see two variables rather than all eight, which helps narrow down the data even further.**
...
**Answer: The expenditure date column changed the order to year, month, date, with dashes between the numbers instead of slashes.**
...
**Answer: The month column is called "date," which reveals the number that corresponds with the month. Instead, it should be classified as a character, so it would read: Month.**
...
**Answer: This code below seems to be creating new data that can be derived from the given data, to answer the unanswered question of what percentage of voters voted for Moore. The code is taking data from the 2022 election to calculate who voted for just Moore and then comparing that to the total number of votes, which can be calculated by adding all the votes for each of the candidates.**
...
**Answer: This change now shows how the candidate compares to other candidates, revealing the percentage of success in the election. The previous data did not include any obvious comparisons to the candidate's overall performance, just how the candidate did in certain districts. Seeing the descending arrangement makes the differences in candidates more obvious.**
...
**Answer: I would say this data is more useful because candidates need to know where the numbers are slacking. By viewing the data in ascending order, we can see what counties are voting for Moore the least.**
...
**mutate(UpperCity = str_to_upper(City))**
...
**ChatGPT Question: List two tidyverse functions that are similar to str_to_upper:**
...
**Answer: This data appears to identify the state's full non-abbreviated name, and put the proper abbreviations so that the two are put as the same value in the data. If no state is identifiable, it will appear as "NA."**
...
**Answer: I believe this answer makes sense. We are able to make the column states and then view the total expenses of the state side-by-side in descending order.**
...
1.  How many students are on the waiting list?

2.  What classes are taught in the Linguistics Department?

3.  What classes are taught by Matthew McLaughlin?
