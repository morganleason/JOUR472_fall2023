...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer: Yes - that I didn't specify the column types.**
...
**Task** Run the following codeblock to glimpse the data. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **Answer: Names are in the "Vendor Name" field and dollar amounts are in the "Amount" field.**
...
**Task** Run the following codeblock to group by the vendor's zip code and show any that are less than five characters in length. How many zip codes do not have five characters? What do you think is happening with this data? **Answer: 549 zip codes are less than five characters.**
...
**Task** Run the following codeblock to count the number of payments. Looking at the result, what jumps out at you as potentially problematic? **Answer: There are categories classified as N/A under the "category" column.**
...
**Task** Run the following code to arrange the grants & loan data so that the smallest amount is first. Describe what you see. **Answer: The loan for Land Preservation Trust is** **\$60, and is the smallest amount. The biggest loan was \$24614.00, granted by the Governor's Office of Crime Control.**
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does the function change the headers? Answer below. **Answer: We renamed the Granter column to source.**
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many duplicates are possible? **Answer: 25 duplicates, or 25 pairs of the same data are possible.**
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **Answer: the data from the spreadsheet is returned and cleaned out of problems, such as a bad zip-code column. You can change the data by creating a new column, such as changing the year to a number to look better, visually.**
...
The zipcode 21202 received the greatest amount of money from the Maryland Tourism Board. This zip code is located in Baltimore Maryland.

```{r}
md_grants_loans |>
  filter(Grantor == "Commerce/Maryland Tourism Board") |>
  arrange(desc(Amount))
```
