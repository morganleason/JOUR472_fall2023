...
**Task** Run the following code to group and count records in the Grantee column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Answer: There appears to be a lot of repeating Grantee locations that are similar values.**
...
**Task** Run the following code to make a copy of the Grantee field called grantee_clean, then use the `key_collision_merge` function from refinr to attempt to standardize similar values. Store it as a new object called cleaned_md_grant_loans Click the black arrow on the table to examine the Grantee and grantee_clean columns. Looking at the results on the first 3 pages only (first 30 rows), name two changes the refinr package made, by comparing Grantee to grantee_clean. **Answer: This change added a new column named grantee_clean. Another change is that now there are grouped values in the grantee column that eliminated similar values that stood alone. All of the original data is still present.**
...
**Task** Run the following code to group by grantee_clean and Grantee, then filter for recipients that start with W. Name three changes the refinr function made in the first 30 results. **Answer: This task cleaned up the data even further and created a column with grantee and grantee_clean. The difference is that there are less things with multiple counts, but not enough.**
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by grantee_clean and Grantee to examine recipients that start with W. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Answer: compared to the previous merge, the counts change for the grantee values. It has become more clustered so that there are less repeat values.**
